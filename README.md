...
def products(i,j):
    right = int(grid[i,j])*int(grid[i,j+1])*int(grid[i,j+2])*int(grid[i,j+3])
    left = int(grid[i,j])*int(grid[i,j-1])*int(grid[i,j-2])*int(grid[i,j-3])
    up = int(grid[i,j])*int(grid[i-1,j])*int(grid[i-2,j])*int(grid[i-3,j])
    down = int(grid[i,j])*int(grid[i+1,j])*int(grid[i+2,j])*int(grid[i+3,j])
    diag = int(grid[i,j])*int(grid[i+1,j+1])*int(grid[i+2,j+2])*int(grid[i+3,j+3])
    return right,left,up,down,diag

for i in range(20):
    for j in range(20):
        try:
            r,l,u,d,diag = products(i,j)
        except IndexError:
            break
```
